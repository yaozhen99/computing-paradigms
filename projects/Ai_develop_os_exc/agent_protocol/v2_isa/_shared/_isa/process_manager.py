...
3. signal_start(task) — 请求看门狗启动新进程
4. find_claude_processes() — 发现所有 Claude 进程
5. _find_own_claude_pid() — 5 级策略自动发现当前进程的 claude.exe PID
"""
...
def handoff(task: str = "Continue pending tasks"):
    """一键交接：压缩上下文 → 启动新进程 → 退休旧进程。"""
    # 1. 启动交接执行器
    my_pid = _find_own_claude_pid() or os.getpid()
    subprocess.Popen(
        [sys.executable, "_isa/handoff_executor.py", "--from-pid", str(my_pid), "--task", task,
