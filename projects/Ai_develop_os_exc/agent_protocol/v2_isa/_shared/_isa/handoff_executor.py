"""
handoff_executor.py — ISA 交接执行器（通用版）
====================================

从 C:\\AI_exchange\\skills\\isa-process-handoff\\ 提炼并参数化。
原版硬编码 quant_system 路径，本版从 _system/project_config.json 读取项目空间路径。
...
# 项目空间路径：从 _system/project_config.json 读取，或从命令行参数获取
_this_dir = Path(__file__).resolve().parent
PROJECT_DIR = _this_dir.parent  # _isa/ 的父目录就是项目空间
...
HANDOFF_FILE = AUDIT_DIR / "handoff.json"
LOG_FILE = PROJECT_DIR / "_logs" / "handoff_executor.log"
...
def build_handoff_prompt(old_pid: int, task: str) -> str:
    ...
    return (
        ...
        f"1. from _isa.process_manager import claim_primary, signal_kill\n"
        f"2. claim_primary()  # auto-discover PID and claim primary role\n"
        f"3. signal_kill({old_pid})  # ask watchdog to kill old process\n"
        f"4. Read _system/system_state.json to restore context\n"
        f"5. Update heartbeat: python _isa/heartbeat_loop.py\n"
        f"6. Check pending tasks and continue execution\n"
        f"7. After tasks done, run: python _isa/heartbeat_loop.py  # keep process alive, do NOT exit\n\n"
        f"Current task: {task}\n"
    )
...
def main():
    ...
    parser.add_argument("--project-dir", type=str, default="", help="Project space directory path")
    ...
    _log(f"Handoff executor started: old_pid={old_pid}, task={task}, project_dir={PROJECT_DIR}")
    ...
    try:
        ...
        # 写交接命令到文件（供剪贴板读取）
        handoff_cmd_file = PROJECT_DIR / "_system" / "handoff_command.txt"
        handoff_cmd_file.parent.mkdir(parents=True, exist_ok=True)
        ...
        launcher_ps1 = PROJECT_DIR / "_system" / "handoff_launcher.ps1"
        ...
        # 2. 等 Claude 进程出现
        from _isa.process_manager import find_claude_processes
        ...
        # 把交接命令复制到剪贴板
        clip_ps1 = PROJECT_DIR / "_system" / "set_clipboard.ps1"
        ...
        subprocess.run(
            ["powershell", "-ExecutionPolicy", "Bypass", "-File", str(clip_ps1)],
            capture_output=True, text=True, timeout=5
        )
        time.sleep(0.5)

        # 激活 PowerShell 窗口（用窗口标题精确匹配）
        activate_ps1 = PROJECT_DIR / "_system" / "activate_window.ps1"
        with open(activate_ps1, "w", encoding="utf-8-sig") as f:
            ...
            f.write('}\n')
        r = subprocess.run(
            ["powershell", "-ExecutionPolicy", "Bypass", "-File", str(activate_ps1)],
    ...
    _write_json(str(HANDOFF_FILE), state)
    ...
    while time.time() - claim_start < claim_timeout:
        token = _read_json(str(ROLE_TOKEN_FILE))
