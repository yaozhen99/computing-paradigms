...
class MockLLMAdapter(LLMAdapter):
    ...
    def set_behavior_mode(
        self,
        mode: MockReviewerBehavior | MockCoderBehavior,
    ) -> None:
        """Configure one-click scenario mode.

        Auto-enables structured_mode=True and pre-configures
        _structured_responses for relevant roles.

        Behavior modes:
        - MockReviewerBehavior.ALWAYS_APPROVE:   reviewer always returns approval intent
        - MockReviewerBehavior.ALWAYS_REJECT:    reviewer always returns veto
        - MockReviewerBehavior.REJECT_THEN_APPROVE: first call veto, subsequent calls approve
        - MockCoderBehavior.GENERATE_SIMPLE:     coder returns simple artifact
        - MockCoderBehavior.GENERATE_WITH_BUG:   coder returns buggy artifact
        """
        ...
        if isinstance(mode, MockReviewerBehavior):
            if mode == MockReviewerBehavior.ALWAYS_APPROVE:
                ...
                # First call: veto, subsequent calls: approve
                self.set_structured_response(
                    role="reviewer",
                    response_type="veto",
                    target_id="task_root",
