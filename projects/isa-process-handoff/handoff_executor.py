...
使用方式：
  python handoff_executor.py --from-pid 12345 --task "Continue pending tasks"
  python handoff_executor.py --from-pid 12345 --task "..." --project-dir "C:\my_project"
"""
...
def build_handoff_prompt(old_pid: int, task: str, project_dir: str = "") -> str:
    ...
    """
    isa_path = os.path.dirname(os.path.abspath(__file__))
    return (
        ...
        f"1. import sys; sys.path.insert(0, r'{isa_path}')\n"
        f"2. from process_manager import claim_primary, signal_kill\n"
        f"3. claim_primary()  # auto-discover PID and claim primary role\n"
        f"4. signal_kill({old_pid})  # ask watchdog to kill old process\n"
        f"5. Read project memory files to restore context\n"
        f"6. Check pending tasks and continue execution\n"
        f"7. After tasks done, run: python {isa_path}/heartbeat_loop.py  # keep process alive, do NOT exit\n\n"
        f"Current task: {task}\n"
    )
...
def main():
    ...
    parser.add_argument("--project-dir", type=str, default="", help="Target project directory (overrides default)")
    ...
    if args.project_dir:
        global PROJECT_DIR, AUDIT_DIR, ROLE_TOKEN_FILE, HANDOFF_FILE, LOG_FILE
    ...
    try:
        ...
        # 2. 等 Claude 进程出现
        from process_manager import find_claude_processes
