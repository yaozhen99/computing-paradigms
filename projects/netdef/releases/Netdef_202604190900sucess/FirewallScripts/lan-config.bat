...
for /f "tokens=2 delims==" %%a in ('type "%iniFile%" ^| findstr /b "TrustedRanges"') do set "trustedRanges=%%a"
...
for /f "tokens=2 delims==" %%a in ('type "%iniFile%" ^| findstr /b "AutoStart"') do set "w2bAuto=%%a"
for /f "tokens=2 delims==" %%a in ('type "%iniFile%" ^| findstr /b "Path"') do set "w2bPath=%%a"
