...
for /f "tokens=2 delims==" %%a in ('type "%iniFile%" ^| find /i "TrustedRanges"') do set "trustedRanges=%%a"
...
for /f "tokens=2 delims==" %%a in ('type "%iniFile%" ^| find /i "Enable"') do set "outboundEnable=%%a"
...
for /f "tokens=2 delims==" %%a in ('type "%iniFile%" ^| find /i "AllowedRanges"') do set "allowedRanges=%%a"
set "allowedRanges=%allowedRanges: =%"
echo [OK] Outbound allowed range(s): %allowedRanges%

:: Read TCP ports to block
for /f "tokens=2 delims==" %%a in ('type "%iniFile%" ^| find /i "BlockTCP"') do set "blockTCP=%%a"
...
for /f "tokens=2 delims==" %%a in ('type "%iniFile%" ^| find /i "BlockUDP"') do set "blockUDP=%%a"
...
for /f "tokens=2 delims==" %%a in ('type "%iniFile%" ^| find /i "AutoStart"') do set "w2bAuto=%%a"
set "w2bAuto=%w2bAuto: =%"
if not defined w2bAuto set "w2bAuto=0"
echo [OK] Wail2Ban Auto-Start: %w2bAuto%

for /f "tokens=2 delims==" %%a in ('type "%iniFile%" ^| find /i "Path"') do set "w2bPath=%%a"
set "w2bPath=%w2bPath: =%"
