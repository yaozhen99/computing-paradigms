...
# ========== State Management ==========
$state = @{}
$mutex = New-Object System.Threading.Mutex($false, "Global\NetdefWail2BanState")
if (Test-Path $config.StateFile) {
    try {
        ...
        if ($acquired) {
            ...
            Write-Log "State file load: mutex timeout, reinitializing..." "WARN"
        }
    }
    catch {
        Write-Log "State file corrupted, reinitializing... ($($_.Exception.Message))" "WARN"
    }
}
...
function Save-State {
    ...
    for ($i = 0; $i -lt $maxRetries; $i++) {
        try {
            ...
            if (-not $acquired) {
                Write-Log "Save-State: mutex wait timeout (attempt $($i+1)/$maxRetries)" "WARN"
                Start-Sleep -Milliseconds $retryDelay
                continue
            }
            try {
                $tempFile = "$($config.StateFile).tmp"
                $state | ConvertTo-Json -Depth 3 | Out-File $tempFile -Encoding utf8 -ErrorAction Stop
                Move-Item -Path $tempFile -Destination $config.StateFile -Force -ErrorAction Stop
                return
            }
            finally {
                $mutex.ReleaseMutex()
            ...
            Write-Log "Save-State failed (attempt $($i+1)/$maxRetries): $_" "WARN"
            if ($i -lt $maxRetries - 1) {
                Start-Sleep -Milliseconds $retryDelay
            }
        }
    }
    Write-Log "Save-State failed after $maxRetries retries" "ERROR"
}
