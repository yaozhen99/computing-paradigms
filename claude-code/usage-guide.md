...
直接运行 `claude` 走全局 `settings.json` 配置，**但全局 `env` 已清空**，需要通过启动脚本指定提供商。
...
| 脚本 | 提供商 | 缺省模型 | 命令 |
|------|--------|---------|------|
| `claude-xfyun` | 讯飞 MaaS | astron-code-latest | `claude-xfyun` |
...
| 终端 | 脚本格式 | 位置 |
|------|---------|------|
| Git Bash / WSL | bash（无后缀） | `C:\Users\yz01\bin\` |
| PowerShell / CMD | `.ps1` | `C:\Users\yz01\AppData\Roaming\npm\` |

> **为什么放 npm 目录？** `C:\Users\yz01\bin` 不在 PowerShell 的用户 PATH 中，PowerShell 找不到那里的脚本。npm 目录已在 PATH 中，跟 `claude` 本体同级，无需改 PATH。

源文件备份：`C:\tower-of-babel\claude-code\scripts\`
...
claude-acw -p "问题"            # 不指定模型，直接传参给 claude
```
...
claude-deepseek -p "问题"            # 不指定模型，直接传参给 claude
```
...
`~/.claude/settings.json` 的 `env` 字段已清空。提供商切换完全由启动脚本的环境变量控制。

如果 `env` 里硬编码了某个提供商的配置，Claude Code 启动时会用它覆盖脚本设的环境变量，导致所有脚本都走同一个提供商。
...
```bash
claude                          # 默认启动（需先设环境变量，否则无提供商）
claude-acw                      # ACW Sonnet 交互模式
claude-deepseek -p "问题"       # DeepSeek 单次提问
claude-xfyun -c                 # 讯飞继续上次对话
claude-acw claude-opus-4-7      # ACW 指定 Opus 模型
```
