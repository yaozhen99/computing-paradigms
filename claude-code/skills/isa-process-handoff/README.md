# ISA Process Handoff — Claude CLI 主备交接方案

> 在 Windows 环境下实现 Claude CLI 进程的主备交接（hot handoff），确保 AI 代理持续运行不中断。
...
| 文件 | 职责 |
|:---|:---|
| `utils_common/process_manager.py` | 进程管理核心：claim_primary、signal_kill、find_claude_processes、_find_own_claude_pid |
| `handoff_executor.py` | 交接执行器：启动新进程 + 输入命令 + 等待确认 |
| `heartbeat_loop.py` | 心跳保活循环：新进程完成任务后持续运行 |
| `watchdog.py` | Python 心跳守护：检查 AI 存活、触发唤醒/熔断 |
| `wake_me.py` | 唤醒脚本：被看门狗调用，启动新 Claude 进程 |
| `_system/watchdog.ps1` | 极简看门狗：处理信号文件 + 心跳超时唤醒 |
| `audit/role_token.json` | 角色令牌：谁在当 primary |
| `audit/handoff.json` | 交接状态：phase + 新旧 PID |
| `_system/system_state.json` | 心跳文件：PowerShell 看门狗读 |
